...
public class MainActivity extends Activity {
    ...
    private PureDashboardView dashboardView;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        dashboardView = new PureDashboardView(this);
    ...
    public synchronized void startCoolerScan() {
        ...
        dashboardView.scanStatusText = "正在搜索智能硬件...";
        dashboardView.postInvalidate();
    ...
    private final ScanCallback scanCallback = new ScanCallback() {
        ...
        public void onScanResult(int callbackType, ScanResult result) {
            ...
            // 兼容各类通用散热器及解包中的工具类设备名前缀
            if (upper.contains("B6X") || upper.contains("B7X") || upper.contains("B8X") || upper.contains("B9X")
                    || upper.contains("COOLER") || upper.contains("FZ") || upper.contains("TOOL")
                    || upper.contains("B6") || upper.contains("B7")) {
                stopCoolerScan();
    ...
    private final BluetoothGattCallback gattCallback = new BluetoothGattCallback() {
        ...
        public void onConnectionStateChange(final BluetoothGatt gatt, int status, int newState) {
            mainHandler.post(() -> {
                if (newState == BluetoothProfile.STATE_CONNECTED) {
                    dashboardView.deviceNameStr = "智能温控舱";
                    dashboardView.isSearchingCooler = false;
                    dashboardView.triggerHaptic(true);
                    dashboardView.postInvalidate();
                    try {
                        Toast.makeText(getApplicationContext(), "硬件已成功连接", Toast.LENGTH_SHORT).show();
                    } catch (Throwable ignored) {}
        ...
        public void onCharacteristicChanged(BluetoothGatt gatt, BluetoothGattCharacteristic characteristic) {
            ...
            if (data != null && data.length > 0) {
                ...
                float hardwareTemp = rawTemp * 0.5f + 5.0f; // 实时冷面温度解析
                mainHandler.post(() -> {
...
// 纯净版自定义座舱视图
class PureDashboardView extends View {
    ...
    public String scanStatusText = "正在搜索智能硬件...";
    ...
    public PureDashboardView(Context context) {
        super(context);
        setClickable(true);
    ...
    private void drawBleSearchOverlay(Canvas canvas, float w, float h) {
        ...
        textPaint.setTypeface(Typeface.DEFAULT_BOLD);
        canvas.drawText(scanStatusText, radarCx, dy + dp(156), textPaint);
